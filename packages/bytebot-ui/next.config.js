...
const nextConfig = {
  ...
  // API proxy to ByteBot agent
  async rewrites() {
    return [
      {
        source: '/api/tasks/:path*',
        destination: `${process.env.BYTEBOT_AGENT_BASE_URL || 'http://localhost:9991'}/tasks/:path*`,
      },
      {
        source: '/api/computer-use/:path*',
        destination: `${process.env.BYTEBOT_DESKTOP_BASE_URL || 'http://localhost:9990'}/computer-use/:path*`,
      },
    ]
  },
